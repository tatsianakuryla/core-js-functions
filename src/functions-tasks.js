...
function getPolynom(...args) {
  return function polynom(x) {
    ...
    return args.reduce((acc, curr, index) => {
      const result = acc + curr * x ** (length - index - 1);
      return result;
    }, 0);
  };
}
...
function logger(func, logFunc) {
  return function wrapper(...args) {
    const argString = args
      ...
      .join(',');
    const fnName = func.name || 'anonymous';
    logFunc(`${fnName}(${argString}) starts`);
    const result = func(...args);
    logFunc(`${fnName}(${argString}) ends`);
...
function getIdGeneratorFunction(startFrom) {
  ...
  return function getId() {
    if (startFrom in idStore) {
      idStore[startFrom] += 1;
      return idStore[startFrom];
    }
    idStore[startFrom] = startFrom;
    return idStore[startFrom];
  };
}
